...
const AdminPanel: React.FC = () => {
  ...
  useEffect(() => {
    axios.get('http://3.138.105.216:8080/api/admin/stats', {
  ...
  useEffect(() => {
    axios.get('http://3.138.105.216:8080/api/admin/users', {
      headers: getAuthHeaders()
    })
    .then(res => {
      setUsers(res.data);
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await axios.delete(`http://3.138.105.216:8080/api/admin/users/${id}`, {
        headers: getAuthHeaders()
      });
